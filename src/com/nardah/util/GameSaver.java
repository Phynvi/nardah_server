...
public final class GameSaver {
	...
	public static void save() {
		new Thread(() -> {
			try(FileWriter fw = new FileWriter("./data/content/game/world.json")) {
				fw.write(GsonUtils.JSON_PRETTY_NO_NULLS.toJson(toJson()));
	...
	public static JsonObject toJson() {
		...
		for(WellOfGoodwill.Contributor contributor : WellOfGoodwill.contributors) {
		...
		object.add("pk-bot-loot", GsonUtils.JSON_PRETTY_ALLOW_NULL.toJsonTree(BotUtility.BOOT_LOOT));
		return object;
	}
	...
	public static void load() {
		new GsonParser("content/game/world") {
			...
			protected void parse(JsonObject object) {
				...
				BotUtility.BOOT_LOOT = GsonUtils.JSON_PRETTY_ALLOW_NULL.fromJson(object.get("pk-bot-loot"), new TypeToken<HashMap<Integer, MutableNumber>>() {
				}.getType());
			}
		}.run();
	}
