...
public final class TaskManager {
	 ...
	 */
	private static final Logger logger = LogManager.getLogger(LoggerType.TASK_SEQUENCING);
		...
		try {
			...
			if(task.isInstant()) {
				try {
					...
					logger.error(String.format("error executing task: %s", task.getClass().getSimpleName()));
					ex.printStackTrace();
					return;
				}
			}
	...
	public void processTasks() {
		...
		for(final ListIterator<Task> itr = active.listIterator(); itr.hasNext(); ) {
			...
			try {
				...
				logger.error(String.format("Error executing task: %s [pendingQueue: %d executionQueue: %d], ", task.getClass().getSimpleName(), pending.size(), active.size()));
				ex.printStackTrace();
				task.cancel();
			}
