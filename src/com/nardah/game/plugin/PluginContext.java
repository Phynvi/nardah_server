package com.nardah.game.plugin;

import com.nardah.game.event.Event;
import com.nardah.game.event.impl.*;
import com.nardah.game.event.impl.log.CommandLogEvent;
import com.nardah.game.event.impl.log.PickupItemLogEvent;
import com.nardah.game.event.listener.PlayerEventListener;
...
public class PluginContext implements PlayerEventListener {
	...
	public boolean accept(Player player, Event event) {
		try {
			...
			logger.error(String.format("player=%s error while handling event: %s", player.getName(), this.getClass().getSimpleName()));
			ex.printStackTrace();
		}
	...
	private boolean handleItemOnNpcEvent(Player player, ItemOnNpcEvent event) {
		if(itemOnNpc(player, event)) {
			if(player.debug) {
				player.send(new SendMessage(String.format("[%s]: item=%d slot=%d mob=%d", this.getClass().getSimpleName(), event.getUsed().getId(), event.getSlot(), event.getMob().id)));
			}
			return true;
		}
		return false;
		...
		switch(event.getType()) {
			case 1:
				if(firstClickNpc(player, event)) {
					if(player.debug) {
						player.send(new SendMessage(String.format("[%s, type=%d]: mob=%d", this.getClass().getSimpleName(), event.getType(), event.getMob().id)));
					}
					return true;
				}
				break;
