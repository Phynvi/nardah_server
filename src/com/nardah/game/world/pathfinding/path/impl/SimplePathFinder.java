...
import static com.nardah.game.world.entity.actor.Direction.NONE;
import static com.nardah.game.world.entity.actor.Direction.getDirection;
...
public final class SimplePathFinder extends PathFinder {
	 ...
	 */
	@Override
	public Path find(Position start, int originWidth, int originLength, Position end, int targetWidth, int targetLength) {
	 ...
	 */
	private Deque<Position> addWalks(Position origin, int originWidth, int originLength, Position target, int targetWidth, int targetLength, Deque<Position> positions) {
	...
	private boolean traversable(Position current, int size, Interactable target, Direction direction) {
		...
		return traversable(current, size, direction) && !Utility.inside(next, size, size, target.getPosition(), target.width(), target.length());
	}
