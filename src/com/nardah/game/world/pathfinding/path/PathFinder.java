package com.nardah.game.world.pathfinding.path;
...
public abstract class PathFinder {
	 ...
	 */
	public abstract Path find(Position origin, int originWidth, int originLength, Position target, int targetWidth, int targetLength);

	public final Path find(Interactable origin, Interactable target) {
		return find(origin.getPosition(), origin.width(), origin.length(), target.getPosition(), target.width(), target.length());
	}
