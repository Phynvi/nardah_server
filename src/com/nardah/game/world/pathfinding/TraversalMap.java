package com.nardah.game.world.pathfinding;
...
import com.nardah.game.world.region.RegionTile;

import java.util.LinkedList;
import java.util.List;
import java.util.Optional;

import static com.nardah.game.world.object.ObjectType.*;
...
public final class TraversalMap {
	...
	public static void set(Region region, int height, int x, int y, int flag) {
		if(region == null)
			region = World.getRegions().getRegion(x, y);
		if(region == null)
			return;
	...
	public static List<Position> getSurroundedTraversableTiles(Position from, int leaderSize, int followerSize) {
		...
		// south
		for(int x = from.getX() - 1; x < from.getX() + leaderSize; x++) {
			Direction d = Direction.getDirection(new Position(x, from.getY() - ((followerSize - 1) + 1), from.getHeight()), from);
			Direction d2 = Direction.getDirection(from, new Position(x, from.getY() - ((followerSize - 1) + 1), from.getHeight()));
		...
		for(int y = from.getY() - 1; y < from.getY() + leaderSize; y++) {
			Direction d = Direction.getDirection(new Position(from.getX() - ((followerSize - 1) + 1), y, from.getHeight()), from);
			Direction d2 = Direction.getDirection(from, new Position(from.getX() - ((followerSize - 1) + 1), y, from.getHeight()));
		...
		// east
		for(int y = from.getY() - 1; y < from.getY() + leaderSize; y++) {
			Direction d = Direction.getDirection(new Position(from.getX() + (leaderSize - 1) + 1, y, from.getHeight()), from);
			Direction d2 = Direction.getDirection(from, new Position(from.getX() + (leaderSize - 1) + 1, y, from.getHeight()));
