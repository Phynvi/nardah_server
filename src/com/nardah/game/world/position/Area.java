...
public abstract class Area {
	...
	private static final ImmutableList<Area> SCHOOL = ImmutableList.of(new SquareArea("School Area", 1730, 5058, 1790, 5112, 2));
	...
	private static final ImmutableList<Area> EVENT_ARENA = ImmutableList.of(new SquareArea("Event Arena", 3118, 3481, 3127, 3487));
	
	// private static final ImmutableList<Area> BOSS_ARENA = ImmutableList.of(new
	// SquareArea("Boss Arena", 3052, 5357, 3027, 5331));
	
	private static final ImmutableList<Area> TOURNAMENT_ARENA = ImmutableList.of(new SquareArea("Tournament Arena", 3317, 3268, 4987, 4932));
	 ...
	 */
	private static final ImmutableList<Area> DUEL_ARENA_LOBBY = ImmutableList.of(new SquareArea("Lobby", 3355, 3262, 3379, 3279), new SquareArea("Lobby Altar", 3374, 3280, 3379, 3286), new SquareArea("Lobby east bank", 3380, 3263, 3384, 3273));
	
	/**
	 * CERB AREA
	 **/
	private static final ImmutableList<Area> CERBERUS = ImmutableList.of(new SquareArea("Cerberus Lair", 1216, 1216, 1278, 1279));
	 ...
	 */
	private static final ImmutableList<Area> MULTI_COMBAT_ZONES = ImmutableList.of(new SquareArea("Lizard shamans", 1408, 3692, 1457, 3729), new SquareArea("Al-kahrid Warrior", 3281, 3158, 3304, 3178), new SquareArea("Rock Crab", 2660, 3711, 2743, 3739), new SquareArea("Arena Zone", 2257, 5309, 2285, 5370), new SquareArea("Wilderness Resource", 3174, 3924, 3196, 3944), new SquareArea("Start of Varrock Wilderness", 3134, 3525, 3327, 3607), new SquareArea("North of GE, near gravestones", 3190, 3648, 3327, 3839), new SquareArea("Near Chaos Elemental", 3200, 3840, 3390, 3967), new SquareArea("Near wilderness agility course", 2992, 3912, 3007, 3967), new SquareArea("West wilderness altar", 2946, 3816, 2959, 3831), new SquareArea("Deep wilderness 1", 3008, 3856, 3199, 3903), new SquareArea("Near wilderness castle", 3008, 3600, 3071, 3711), new SquareArea("North of varrock lumbermill", 3072, 3608, 3327, 3647), new SquareArea("Pest control", 2624, 2550, 2690, 2619), new SquareArea("Fight caves", 2371, 5062, 2422, 5117), new SquareArea("Fight arena", 2896, 3595, 2927, 3630), new SquareArea("Scorpia lair", 3218, 10329, 3248, 10353), new SquareArea("Corporeal Beast lair", 2964, 4360, 3001, 4399), new SquareArea("Vorkath", 2261, 4054, 2289, 4048), new SquareArea("Dagannoth lair", 2890, 4425, 2942, 4466));
	
	private static final ImmutableList<SquareArea> DUEL_ARENAS = ImmutableList.of(new SquareArea("Arena", 3327, 3203, 3389, 3290));
	
	private static final ImmutableList<SquareArea> DUEL_OBSTICLE_ARENAS = ImmutableList.of(new SquareArea("South East Obstacle Arena", 3362, 3205, 3390, 3221), new SquareArea("West Obstacle Arena", 3331, 3224, 3359, 3240), new SquareArea("North East Obstacle Arena", 3362, 3243, 3390, 3259));
	...
	private static final ImmutableList<Area> NEW_WILDERNESS = ImmutableList.of(new SquareArea("Wilderness 4", 3392, 2944, 3480, 3136), new SquareArea("Wilderness 5", 3392, 3136, 3435, 3163) , new SquareArea("Wilderness 6", 3414, 3159, 3434, 3170) , new SquareArea("Wilderness 7", 3435, 3136, 3491, 3155), new SquareArea("Wilderness 8", 3436, 3156, 3447, 3161));
	...
	public static boolean inMulti(Entity entity) {
		...
		return inEventArena(entity) || inTournamentArena(entity) || inkolodionArena(entity) || inKraken(entity) || inKingBlackDragon(entity) || inGodwars(entity) || inZulrah(entity);
	}
	...
	public static boolean inCyclops(Interactable entity) {
		return inArea(entity, new Position(2847, 3532, 2), new Position(2876, 3542, 2)) || inArea(entity, new Position(2838, 3543, 2), new Position(2877, 3577, 2));
	}
	...
	public static boolean inBattleRealm(Interactable entity) {
		for(Area zone : BattleRealmCallers.getAreas()) {
			if(zone.inArea(entity.getPosition())) {
				return true;
			}
		}
	...
	public static boolean inBattleRealmMap(Position position) {
		...
		return BattleRealmCallers.getAreas().get(0).inArea(position);
	}
