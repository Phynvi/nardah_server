package com.nardah.game.world.items.containers.equipment;

import com.google.common.collect.ImmutableSet;
import com.nardah.content.emote.Skillcape;
import com.nardah.game.world.entity.actor.ActorAnimation;
...
public final class Equipment extends ItemContainer {
	 ...
	 */
	public static final int HEAD_SLOT = 0, HELM_SLOT = 0, CAPE_SLOT = 1, AMULET_SLOT = 2, WEAPON_SLOT = 3, CHEST_SLOT = 4, SHIELD_SLOT = 5, LEGS_SLOT = 7, HANDS_SLOT = 9, FEET_SLOT = 10, RING_SLOT = 12, ARROWS_SLOT = 13;
	
	/**
	 * Equipment bonus constants.
	 */
	public static final int STAB_OFFENSE = 0, SLASH_OFFENSE = 1, CRUSH_OFFENSE = 2, MAGIC_OFFENSE = 3, RANGED_OFFENSE = 4, STAB_DEFENSE = 5, SLASH_DEFENSE = 6, CRUSH_DEFENCE = 7, MAGIC_DEFENSE = 8, RANGED_DEFENSE = 9, STRENGTH_BONUS = 10, RANGED_STRENGTH = 11, MAGIC_STRENGTH = 12, PRAYER_BONUS = 13;
	...
	private void onEquip(Item item) {
		...
		if(type == EquipmentType.SHIELD || type == EquipmentType.WEAPON) {
			...
			item.getRangedDefinition().filter(def -> def.getType().equals(RangedWeaponType.THROWN)).ifPresent(def -> retrieve(ARROWS_SLOT).ifPresent(this::removeBonus));
			...
			getWeapon().getRangedDefinition().filter(def -> def.getType().equals(RangedWeaponType.THROWN)).ifPresent(def -> removeBonus(item));
		}
	}
	...
	private void onRemove(Item item) {
		...
		if(type == EquipmentType.SHIELD || type == EquipmentType.WEAPON) {
			...
			if(isBlowpipe || item.getRangedDefinition().filter(def -> def.getType().equals(RangedWeaponType.THROWN)).isPresent()) {
			...
			if(isBlowpipe || getWeapon().getRangedDefinition().filter(def -> def.getType().equals(RangedWeaponType.THROWN)).isPresent()) {
				addBonus(item);
			}
	...
	public void updateRangedEquipment() {
		if(!hasWeapon() || !getWeapon().getRangedDefinition().isPresent()) {
			if(hasWeapon() && getWeapon().matchesId(12_926) && player.blowpipeDarts != null) {
				...
				player.rangedAmmo = retrieve(ARROWS_SLOT).map(arrow -> RangedAmmunition.find(getWeapon(), arrow)).orElse(null);
			}
	...
	public void updateAnimation() {
		int stand = ActorAnimation.PLAYER_STAND;
		int walk = ActorAnimation.PLAYER_WALK;
		int run = ActorAnimation.PLAYER_RUN;
	...
	public static boolean isWearingDFS(Player player) {
		return player.equipment.hasShield() && (player.equipment.getShield().getId() == 11283 || player.equipment.getShield().getId() == 11284);
	}
	...
	private final class EquipmentListener extends ItemContainerAdapter {
		...
		public void itemUpdated(ItemContainer container, Optional<Item> oldItem, Optional<Item> newItem, int index, boolean refresh) {
			...
			boolean weapon = oldItem.filter(item -> item.getWeaponInterface() != null).orElse(newItem.filter(item -> item.getWeaponInterface() != null).orElse(null)) != null;
