package com.nardah.game.world.items.containers.bank;
...
import static com.nardah.Config.CURRENCY;
...
public class BankVault {
	 ...
	 */
	public void value() {
		player.send(new SendMessage("You currently have " + Utility.formatDigits(container) + " coin" + (container == 1 ? "" : "s") + " stored inside your bank vault.", MessageColor.DARK_RED));
		player.speak("<icon=15> I currently have " + Utility.formatDigits(container) + " coin" + (container == 1 ? "" : "s") + " stored inside your bank vault.");
	}
	...
	public boolean add(long amount, boolean message) {
		...
		container += amount;
		if(message) {
			player.send(new SendMessage(Utility.formatDigits(amount) + " coins have been added into your bank vault.", MessageColor.DARK_BLUE));
		}
		return true;
	}
	...
	public void deposit(int amount) {
		...
		if(Long.MAX_VALUE == container) {
			player.send(new SendMessage("Your vault is currently full and can no longer hold any more coins!", MessageColor.RED));
			return;
		}
		...
		container += amount;
		player.bank.refresh();
		player.send(new SendMessage("You now have " + Utility.formatDigits(container) + " coins stored inside your bank vault.", MessageColor.DARK_RED));
	}
	...
	public void withdraw(long amount) {
		...
		if(container < 0) {
			World.sendMessage(player.getName() + " HAS POTENTIALLY FOUND AND ABUSED A DUPE. STAFF INVESTIGATE IMMEDIATELY.");
			World.sendMessage(player.getName() + " HAS POTENTIALLY FOUND AND ABUSED A DUPE. STAFF INVESTIGATE IMMEDIATELY.");
			World.sendMessage(player.getName() + " HAS POTENTIALLY FOUND AND ABUSED A DUPE. STAFF INVESTIGATE IMMEDIATELY.");
			World.sendMessage(player.getName() + " HAS POTENTIALLY FOUND AND ABUSED A DUPE. STAFF INVESTIGATE IMMEDIATELY.");
		}
		...
		if(player.inventory.add(new Item(CURRENCY, (int) amount))) {
			container -= amount;
			player.send(new SendMessage("You have successfully withdrawn " + Utility.formatDigits(amount) + " coin" + (amount == 1 ? "" : "s") + "."));
			player.bank.refresh();
		}
	}
}
