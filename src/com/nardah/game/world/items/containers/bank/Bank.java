...
public class Bank extends ItemContainer {
	 ...
	 */
	public void refresh() {
		player.send(new SendString("Bank Vault: <col=FF5500>" + Utility.formatPrice(player.bankVault.container) + "</col>", 60080));
	...
	public void deposit(int slot, int amount) {
		...
		if(id == 11941 && !player.lootingBag.isEmpty()) {
	...
	public void withdraw(int itemId, int slot, int amount) {
		...
		if(remove(item.getId(), amount)) {
			...
			// when an item is taken out of the bank completely, it removes one amount from
			// the tab amounts array
	...
	public void depositeInventory() {
		int tab = bankTab;
		while(tab > 0 && tabAmounts[tab - 1] <= 0)
			tab--;
	...
	private void itemToTab(int slot, int toTab) {
		...
		/*
		 * The tab to the left of the chosen tab is empty, so don't create a new tab.
		 */
	 ...
	 * <p>
	 * An opcode of 0 performs a swap operation, with {@code from} being the origin
	 * slot and {@code to} being the destination slot.
	 * </p>
	 * <p>
	 * An opcode of 1 performs an insert operation, with {@code from} being the
	 * origin slot and {@code to} being the destination slot.
	 * </p>
	 * <p>
	 * An opcode of 2 moves an item from one tab to another, with {@code from} being
	 * the origin <b>slot</b> and {@code to} being the destination <b>tab</b>.
	 * </p>
