...
public class DonatorDeposit extends ItemContainer {
	...
	public void confirm() {
		...
		if(!stopwatch.elapsed(2, TimeUnit.MINUTES)) {
			player.dialogueFactory.sendStatement("You can only deposit items every 2 minutes", "Elapsed time: " + stopwatch.elapsedTime(TimeUnit.MINUTES) + " (" + stopwatch.elapsedTime(TimeUnit.SECONDS) + " seconds)").execute();
			return;
		}
