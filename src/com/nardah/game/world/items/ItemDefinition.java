package com.nardah.game.world.items;
...
import com.nardah.util.parser.JsonSaver;
...
public class ItemDefinition {
	...
	private ItemDefinition(int id, String name) {
		...
		this.requirements = CombatConstants.EMPTY_REQUIREMENTS;
		this.bonuses = CombatConstants.EMPTY_BONUSES;
	}
	...
	public static GsonParser createParser() {
		return new GsonParser("def/item/item_definitions", false) {
			...
			protected void parse(JsonObject data) {
				...
				for(int index = 0; index < CombatConstants.SKILL_REQUIREMENT_CONFIG_FIELD_NAMES.length; index++) {
					String skillRequirement = CombatConstants.SKILL_REQUIREMENT_CONFIG_FIELD_NAMES[index];
					if(data.has(skillRequirement)) {
						if(definition.requirements == CombatConstants.EMPTY_REQUIREMENTS) {
							definition.requirements = new int[CombatConstants.EMPTY_REQUIREMENTS.length];
						}
						definition.requirements[index] = data.get(skillRequirement).getAsInt();
					}
				}
				
				for(int index = 0; index < CombatConstants.BONUS_CONFIG_FIELD_NAMES.length; index++) {
					String bonusName = CombatConstants.BONUS_CONFIG_FIELD_NAMES[index];
					if(data.has(bonusName)) {
						if(definition.bonuses == CombatConstants.EMPTY_BONUSES) {
							definition.bonuses = new int[CombatConstants.EMPTY_BONUSES.length];
						}
	...
	public static void dump(ItemDefinition[] definitions, String path) {
		...
		for(ItemDefinition definition : definitions) {
			if(definition == null)
				continue;
			if(definition.name.equals("null"))
				continue;
			...
			if(definition.weaponInterface != WeaponInterface.UNARMED) {
				saver.current().addProperty("weapon-interface", definition.weaponInterface.name());
			}
			
			for(int index = 0; index < CombatConstants.SKILL_REQUIREMENT_CONFIG_FIELD_NAMES.length; index++) {
				if(definition.requirements[index] > 0)
					saver.current().addProperty(CombatConstants.SKILL_REQUIREMENT_CONFIG_FIELD_NAMES[index], definition.requirements[index]);
			}
			
			for(int index = 0; index < CombatConstants.BONUS_CONFIG_FIELD_NAMES.length; index++) {
				if(definition.bonuses[index] != 0)
					saver.current().addProperty(CombatConstants.BONUS_CONFIG_FIELD_NAMES[index], definition.bonuses[index]);
			}
	 ...
	 * @return The item definition for the item id, or null if the item id is out of
	 * bounds.
	 */
	...
	/**
	 * Gets the noted id.
	 * @return The noted id of this item if this item is un-noted, or the original
	 * item id if it is already un-noted.
	 */
	 ...
	 * @return The un-noted id of this item if this item is noted, or the original
	 * item id if it is already un-noted.
	 */
