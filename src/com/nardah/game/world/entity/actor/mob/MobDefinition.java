...
/**
 * Contains the mob definitions.
 * @author Michael | Chex
 */
public class MobDefinition {
	
	/**
	 * The array of mob definitions.
	 */
	public static final MobDefinition[] DEFINITIONS = new MobDefinition[Config.NPC_DEFINITION_LIMIT + 1];
	...
	public MobDefinition(int id, String name) {
		this.id = id;
		this.name = name;
		this.combatLevel = 0;
		this.size = 1;
		...
		this.skills = CombatConstants.EMPTY_SKILLS;
	...
	public static GsonParser createParser() {
		return new GsonParser("def/mob/npc_definitions", false) {
			...
			protected void parse(JsonObject data) {
				...
				MobDefinition definition = new MobDefinition(id, name);
				...
				for(int index = 0; index < CombatConstants.SKILL_FIELD_NAMES.length; index++) {
					String skillName = CombatConstants.SKILL_FIELD_NAMES[index];
					if(data.has(skillName)) {
						if(definition.skills == CombatConstants.EMPTY_SKILLS) {
							definition.skills = new int[CombatConstants.EMPTY_SKILLS.length];
						}
	...
	/**
	 * Gets a mob definition from the definition array.
	...
	public static MobDefinition get(int id) {
		...
		if(DEFINITIONS[id] == null)
			return new MobDefinition(id, "null");
	...
	public boolean equals(Object obj) {
		return obj == this || (obj instanceof MobDefinition && ((MobDefinition) obj).id == id);
	}
