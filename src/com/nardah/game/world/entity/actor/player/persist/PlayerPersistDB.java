...
import com.nardah.game.service.PostgreService;
...
public final class PlayerPersistDB implements PlayerPersistable {
	...
	private static final PlayerDBProperty[] DB_PROPERTIES = {
			...
			new PlayerDBProperty("json_properties") {
				...
				void read(Player player, JdbcSession session) throws SQLException {
					session.sql("SELECT member_id, data FROM player.player WHERE member_id = ?").set(player.getMemberId()).select((rset, stmt) -> {
						if(rset.next()) {
				...
				void write(Player player, JdbcSession session) throws SQLException {
					...
					session.sql("INSERT INTO player.player(member_id, data) VALUES (?, ?::json) ON CONFLICT (member_id) DO UPDATE SET data = excluded.data, last_save = DEFAULT").set(player.getMemberId()).set(GSON.toJson(properties)).execute();
				}
			},
			...
			new PlayerDBProperty("host") {
				@Override
				void read(Player player, JdbcSession session) throws SQLException {
					session.sql("SELECT host, time FROM player.host where player_id = ? ORDER BY time LIMIT 1").set(player.getMemberId()).select((rset, stmt) -> {
						if(rset.next()) {
				...
				void write(Player player, JdbcSession session) throws SQLException {
					...
					session.sql("INSERT INTO player.host(host, player_id) VALUES (?, ?)").set(player.getSession().get().getHost()).set(player.getMemberId()).execute();
				}
			},
			...
			new PlayerDBProperty("skills") {
				@Override
				void read(Player player, JdbcSession session) throws SQLException {
					session.sql("SELECT id, current_level, xp, player_id FROM player.skill WHERE player_id = ?").set(player.getMemberId()).select((rset, stmt) -> {
				...
				void write(Player player, JdbcSession session) throws SQLException {
					for(Skill skill : player.skills.getSkills()) {
						session.sql("INSERT INTO player.skill(id, current_level, xp, player_id) VALUES (?, ?, ?::double precision, ?) ON CONFLICT (id, player_id) DO UPDATE SET current_level = excluded.current_level, xp = excluded.xp").set(skill.getSkill()).set(skill.getLevel()).set(skill.getExperience()).set(player.getMemberId()).execute();
					}
				}
			},
			...
			new PlayerDBProperty("items") {
				...
				void read(Player player, JdbcSession session) throws SQLException {
					...
					// bank
					session.sql("SELECT item_id, amount, slot FROM player.bank WHERE player_id = ? AND item_id IS NOT NULL ORDER BY slot").set(player.getMemberId()).select((rset, stmt) -> {
						while(rset.next()) {
					...
					// equipment
					session.sql("SELECT item_id, amount, slot FROM player.equipment WHERE player_id = ?").set(player.getMemberId()).select((rset, stmt) -> {
						while(rset.next()) {
					...
					// inventory
					session.sql("SELECT item_id, amount, slot FROM player.inventory WHERE player_id = ?").set(player.getMemberId()).select((rset, stmt) -> {
						while(rset.next()) {
							Item item = new Item(rset.getInt("item_id"), rset.getInt("amount"));
				...
				void write(Player player, JdbcSession session) throws SQLException {
					...
					for(int slot = 0; slot < player.bank.getItems().length; slot++) {
						...
						session.sql("INSERT INTO player.bank(item_id, amount, slot, player_id) VALUES (?, ?, ?, ?) ON CONFLICT (slot, player_id) DO UPDATE SET item_id = excluded.item_id, amount = excluded.amount").set(item.getId()).set(item.getAmount()).set(slot).set(player.getMemberId()).execute();
					...
					for(int slot = 0; slot < player.equipment.getItems().length; slot++) {
						...
						session.sql("INSERT INTO player.equipment(item_id, amount, slot, player_id) VALUES (?, ?, ?, ?) ON CONFLICT (slot, player_id) DO UPDATE SET item_id = excluded.item_id, amount = excluded.amount").set(item.getId()).set(item.getAmount()).set(slot).set(player.getMemberId()).execute();
					...
					for(int slot = 0; slot < player.inventory.getItems().length; slot++) {
						...
						session.sql("INSERT INTO player.inventory(item_id, amount, slot, player_id) VALUES (?, ?, ?, ?)").set(item.getId()).set(item.getAmount()).set(slot).set(player.getMemberId()).execute();
					}
	...
	public LoginResponse load(Player player, String expectedPassword) {
		try {
			...
			boolean exists = session.sql("SELECT EXISTS(SELECT 1 FROM player.player WHERE member_id = ?)").set(player.getMemberId()).select(new SingleOutcome<>(Boolean.class));
			...
			logger.error(String.format("Failed to login new player=%s", player.getName()));
			ex.printStackTrace();
		...
		try {
			...
			logger.error(String.format("Error loading player=%s", player.getName()));
			ex.printStackTrace();
			return LoginResponse.COULD_NOT_COMPLETE_LOGIN;
		}
