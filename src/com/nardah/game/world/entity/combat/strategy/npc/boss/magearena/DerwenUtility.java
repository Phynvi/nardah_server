...
public class DerwenUtility {
	...
	public static Mob generatederwenSpawn() {
		SpawnData spawn = SpawnData.generate();
		Mob derwen = new Mob(7858, spawn.position, 10, Direction.NORTH);
	...
	public static void defeated(Mob jusiticar, Player player) {
		...
		if(hasClan) {
			...
			World.sendMessage("<col=8714E6> Derwen has been defeated by " + player.getName() + ", a clan member of " + player.clanChannel.getName() + "!");
			player.clanChannel.message("Hell yeah boys! We just killed jusiticar!! We earned 10,000 EXP & 5 CP!");
		} else {
			World.sendMessage("<col=8714E6> Derwen has been defeated by " + player.getName() + ", a solo individual with balls of steel!");
		}
