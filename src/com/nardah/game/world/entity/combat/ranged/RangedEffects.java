...
public enum RangedEffects {
	OPAL_BOLTS(new CombatImpact() {
		...
		@Override
		public boolean canAffect(Actor attacker, Actor defender, Hit hit) {
			if(defender.isPlayer() && defender.getPlayer().equipment.containsAny(1383, 1395, 1403)) {
				return false;
			}
		...
		@Override
		public boolean canAffect(Actor attacker, Actor defender, Hit hit) {
			if(attacker.getCurrentHealth() / 5 <= 0)
				return false;
			if(defender.isPlayer() && !RandomUtils.success(0.11))
				return false;
			if(defender.isNpc() && !RandomUtils.success(0.06))
				return false;
		...
		@Override
		public boolean canAffect(Actor attacker, Actor defender, Hit hit) {
			if(defender.isPlayer() && !RandomUtils.success(0.05))
				return false;
			if(defender.isNpc() && !RandomUtils.success(0.10))
				return false;
			return hit.isAccurate() && hit.getDamage() > 0;
		}
