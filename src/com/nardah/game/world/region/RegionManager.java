...
public class RegionManager {
		...
		position.getRegion().getSurroundingRegions().ifPresent(regions -> {
			for(Region region : regions) {
				for(Player other : region.getPlayers(position.getHeight())) {
					if(other == null)
						continue;
					if(Utility.getDistance(other, position) > distance)
						continue;
	 ...
	 */
	public List<Mob> getLocalNpcs(Actor actor) {
		List<Mob> localMobs = new LinkedList<>();
		for(Region region : getSurroundingRegions(actor.getPosition())) {
			for(Mob mob : region.getNpcs(actor.getHeight())) {
				if(actor.getPosition().isWithinDistance(mob.getPosition(), Region.VIEW_DISTANCE)) {
					localMobs.add(mob);
				}
			}
		}
