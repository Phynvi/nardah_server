...
public class Region {
	 ...
	 */
	private final RegionBlock[] blocks = new RegionBlock[Position.HEIGHT_LEVELS];
	...
	public static int getActiveObjectsForId(Player player, int objectId) {
		return (int) ACTIVE_OBJECT.values().stream().filter(o -> o.getId() == objectId).count();
	}
	...
	/**
	 * @return the mobs in this region
	 */
	public Collection<Mob> getNpcs(int height) {
		return getBlock(height).getMobs();
	}
	...
	/**
	 * Adds an mob to this region.
	 */
	public void addNpc(Mob mob) {
		getBlock(mob.getHeight()).addNpc(mob);
	}
	
	/**
	 * Removes an mob from this region.
	 */
	public void removeNpc(Mob mob) {
		getBlock(mob.getHeight()).removeNpc(mob);
	}
	...
	public boolean containsNpc(int height, Mob mob) {
	...
	private RegionBlock getBlock(int height) {
		if(height > Position.HEIGHT_LEVELS || height < 0) {
			throw new IllegalArgumentException("Height is out of bounds. Received (" + height + "), expected range [0, " + Position.HEIGHT_LEVELS + "].");
		}
