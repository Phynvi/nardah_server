...
import com.nardah.game.world.object.*;
	...
	private void parseGameObject(Region region, ByteBuffer buf, int x, int y, Set<Integer> downHeights) {
		...
		while(true) {
			...
			while(true) {
				...
				if(Region.SKIPPED_OBJECTS.contains(pos)) {
				...
				} else if(Area.inBattleRealmMap(pos) && (type.get().getGroup() == ObjectGroup.INTERACTABLE_OBJECT || type.get() == ObjectType.STRAIGHT_WALL) && !staticObject.getDefinition().getName().equals("null")) {
					...
					region.skip(staticObject);
					BattleRealmObjects.battleRealmObjects.add(new BattleRealmObjects.ObjectArgs(staticObject.getId(), pos, staticObject.getDirection(), type.get()));
				} else {
