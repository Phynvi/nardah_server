...
public class MageArena2 extends Quest {
	...
	public void sendDialogues(DialogueFactory factory) {
		...
		switch(getStage(player)) {
			case 0:
				factory.sendPlayerChat("Hey there...").sendNpcChat(4398, Expression.ALMOST_CRYING, "Oh #name, I need your help!", "The three Demi gods have escaped their prison!", "I need your help to slay them, they're causing havoc!", "Brave warrior, Could you please help me?").sendOption("Sure", () -> {
					setStage(player, 1);
					player.interfaceManager.close();
					player.dialogueFactory.sendDialogue(this);
				}, "No, I'm too busy right now", () -> {
					factory.sendPlayerChat("No, I'm too busy right now").sendNpcChat(4398, Expression.ALMOST_CRYING, "*sigh*", "If you change your mind I will be here.");
				}).execute();
				break;
			case 1:
				...
				if(player.inventory.containsAny(needed) && !player.inventory.containsAll(needed)) {
					factory.sendPlayerChat("So what do you need me to do?").sendNpcChat(4398, Expression.HAPPY, "I need the following items to banish the evil spirits:", "1x Saradomin Light, 1x Zamorak Symbol and 1x Guthix symbol.", "You can gather these items by killing the demi gods physical forms!").sendNpcChat(4398, Expression.ANXIOUS, "You can gather these items from the bosses.", "they spawn every 60-90 Minutes At Random Locations!").sendPlayerChat("Alright, I will be right back.").execute();
					return;
				}
				...
				factory.sendPlayerChat("So what do you need me to do?").sendNpcChat(4398, Expression.HAPPY, "I need the following items to banish their spirits:", "1x Saradomin Light, 1x Zamorak Symbol and 1x Guthix symbol.").sendPlayerChat("Alright, I will be right back.").execute();
				break;
			case 2:
				factory.sendNpcChat(4398, Expression.HAPPY, "Thank you so much #name!", "Please take this as a reward").onAction(() -> {
					complete(player);
				}).execute();
				break;
			default:
				factory.sendStatement("You have completed the quest and the", "priest no longer has anything to say.").execute();
				break;
		}
	}
	...
	public final static Item[] REWARD_ITEMS = {new Item(13333, 1), new Item(13335, 1), new Item(13331, 1), new Item(12638, 1), new Item(12637, 1), new Item(12639, 1), new Item(6760, 1), new Item(6762, 1), new Item(6764, 1)};

	@Override
	protected boolean clickNpc(Player player, NpcInteractionEvent event) {
		if(event.getOpcode() == 0 && event.getMob().id == 4398) {
			player.dialogueFactory.sendDialogue(this);
			return true;
		}
		return false;
