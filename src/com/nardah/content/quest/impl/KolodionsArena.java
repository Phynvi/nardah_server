...
import com.nardah.content.activity.impl.magearena.MageArena;
...
public class KolodionsArena extends Quest {
	...
	public void sendDialogues(DialogueFactory factory) {
		...
		switch(getStage(player)) {
			case 0:
				...
				factory.sendNpcChat(1603, Expression.AFFLICTED, "But are you worthy to beat my arena?", "Only the strongest of them all can do it.", "And quite frankly, I don't think you have what it takes!");
				factory.sendPlayerChat("First off, I don't care what you think about me.", "You are an NPC, you are scripted.", "Secondly, I can do anything! So bring it.");
				factory.sendNpcChat(1603, Expression.LAUGH, "Hahaha! We will see who is right in the end.", "If you beat my arena I will reward you handsomely.", "If you fail! My curse will be placed upon you.");
				factory.onAction(() -> {
					setStage(player, 1);
					refresh(player);
			...
			case 1:
				...
				break;
			case 2:
				factory.sendNpcChat(1602, Expression.HAPPY, "I heard the news but I didn't think it was true,", "You've actually beaten Kolodion's arena!", "Congratulations, here is your reward.").execute().onAction(() -> {
					complete(player);
				});
				break;
		}
	...
	protected boolean clickNpc(Player player, NpcInteractionEvent event) {
		if(event.getOpcode() == 0 && event.getMob().id == 1603) {
			if(getState(player) == QuestState.COMPLETED) {
				...
				DialogueFactory factory = player.dialogueFactory;
				factory.sendNpcChat(1603, Expression.HAPPY, "You proved me wrong #name!", "Jump into that sparkling pool for a reward.");
				factory.execute();
				return true;
			}
		...
		if(event.getOpcode() == 0 && event.getMob().id == 1602) {
			if(getState(player) == QuestState.COMPLETED) {
				DialogueFactory factory = player.dialogueFactory;
				factory.sendNpcChat(1602, Expression.HAPPY, "Hello adventurer.", "Would you like to view my shop?");
				factory.sendOption("Yes", () -> Store.STORES.get("Kolodion's Arena Store").open(player), "No", factory::clear);
				factory.execute();
				return true;
			} else if(getStage(player) != 2) {
				DialogueFactory factory = player.dialogueFactory;
