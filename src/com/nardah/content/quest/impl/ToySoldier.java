...
import com.nardah.net.packet.out.SendMinimapState;
import com.nardah.net.packet.out.SendMinimapState.MinimapState;
...
public class ToySoldier extends Quest {
	...
	public void sendDialogues(DialogueFactory factory) {
		...
		switch(getStage(player)) {
			...
			case 1:
				if(npc == 1919) {
					factory.sendPlayerChat("Okay, I will help you find Billy.", " Where was the last place you saw him?").sendNpcChat(npc, Expression.CALM_TALK, "The last time I was playing with ", "Billy was at Barbarian village.").sendPlayerChat("What does Billy look like?").sendNpcChat(npc, Expression.CALM_TALK, "He has a black hat and a red jacket.").sendPlayerChat("I will be right back.").sendNpcChat(npc, Expression.HAPPY, "Thanks so much mister!").execute();
				} else if(npc == 3926) {
					factory.sendNpcChat(npc, Expression.CALM_TALK, "Welcome adventurer,", "I am Brundt the Chieftain.", "How may I help you?").sendPlayerChat("I was wondering if you happened to find a toy,", "it had a black hat and a red jacket.").sendNpcChat(npc, Expression.CALM_TALK, "Oh yes! It's quite a lovely toy. ", "I can't wait to give it to my new born daughter.").sendPlayerChat(Expression.ANGRY, "That's not yours! A little boy lost his toy and he", "is very saddened about it.").sendNpcChat(npc, Expression.HAPPY, "Oh that's terrible.", "However unless you have a replacement toy, this is mine.").sendPlayerChat("Do you think I carry around a replacement toy?").sendNpcChat(npc, Expression.HAPPY, "Mmmm..", "Well then go get one for me.").sendPlayerChat("From where exactly?").sendNpcChat(npc, Expression.HAPPY, "My good friend Zeke can help you.", "He runs a Scimitar shop in Al-kahrid.").execute().onAction(() -> {
						setStage(player, 2);
			...
			case 2:
				if(npc == 527) {
					factory.sendNpcChat(npc, Expression.HAPPY, "Greetings warrior, ", "would you like to purchase a fine crafted Scimitar? ").sendOption("Talk about Toy Soldier", () -> {
						setStage(player, 3);
						player.dialogueFactory.sendDialogue(this);
					}, "View shop", () -> {
			...
			case 3:
				if(npc == 527) {
					factory.sendPlayerChat("I was sent by Brundt the Chieftain, he is seeking", "a toy for his daughter,").sendNpcChat(npc, Expression.CALM_TALK, "Brundt had his daughter! That's wonderful news. ", "Yes wonderful... A toy he seeks? ", " Yes I know of a great toy!").sendNpcChat(npc, Expression.CALM_TALK, "Unfortunately the toy is missing the most ", "important piece. If you can bring me it ", "I can build the toy.").sendPlayerChat("What do you need and where can I get it?").sendNpcChat(npc, Expression.CALM_TALK, "I need a white pearl. Finding it may be hard though...", "If my memory serves, there is a pearl hidden inside a", "chest at 55 wilderness. It's locked up tight so bring", "some lock picks. Oh and it's haunted by a ghost. Good luck!").execute().onAction(() -> {
						setStage(player, 4);
						player.interfaceManager.close();
					});
				}
			...
			case 4:
				if(npc == 527) {
					if(player.inventory.contains(4485)) {
						factory.sendPlayerChat("I Killed the barbarian spirit and retrieved the pearl!").sendNpcChat(npc, Expression.CALM_TALK, "Wonderful news! Give it here.").sendStatement("You have over the white pearl to Zeke").execute().onAction(() -> {
			...
			case 5:
				if(npc == 3926) {
					if(player.inventory.contains(7763)) {
						factory.sendNpcChat(npc, Expression.CALM_TALK, "Have you talked to Zeke?").sendPlayerChat("Yes and he gave me a doll. Can I get Billy back now?").sendNpcChat(npc, Expression.CALM_TALK, "Yes, a deal is a deal. Unfortunately I do not", "have Billy.").sendPlayerChat(Expression.ANGRY, "WHAT!?", "Why did you make me go get you a doll then!?").sendNpcChat(npc, Expression.CALM_TALK, "I needed a toy for my daughter and I can't leave this", "place. We are all held hostige by the demon spiri-").sendPlayerChat(Expression.ANGRY, "I don't have time for your bullshit.", "Take your stupid doll, I am going to find Billy myself.").execute().onAction(() -> {
			...
			case 6:
				if(npc == 1919 && player.inventory.contains(7759)) {
					factory.sendNpcChat(npc, Expression.CALM_TALK, "Did you find Billy mister?").sendPlayerChat("Yeah kid, I did. It was a lot of work but I found him.").sendNpcChat(npc, Expression.CALM_TALK, "Oh thank you so much!", "I found these scrolls when I was playing and I don't", "know what to do with them. Here, you have them.").execute().onAction(() -> {
			...
			case 7:
				if(npc == 1919) {
					factory.sendStatement("You have completed the quest and the", "boy no longer is interested in speaking to you.").execute();
				} else if(npc == 3926) {
					factory.sendStatement("There is no point in speaking to this idiot anymore.").execute();
				} else if(npc == 527) {
					factory.sendNpcChat(npc, Expression.HAPPY, "Greetings warrior, ", "would you like to purchase a fine crafted Scimitar? ").sendOption("Yes", () -> {
					}, "No", () -> {
						player.interfaceManager.close();
					}).execute();
				}
	...
	protected boolean clickNpc(Player player, NpcInteractionEvent event) {
		...
		if(event.getOpcode() == 0 && Arrays.stream(npc).anyMatch($it -> event.getMob().id == $it)) {
			player.attributes.set("QUEST_ENTITY_KEY", event.getMob().id);
			player.dialogueFactory.sendDialogue(this);
			return true;
		}
		return false;
	...
	private void clickChest(Player player, GameObject object) {
		if(player.quest.getStage()[index()] == 4 && !player.inventory.contains(4485)) {
			...
			Mob spirit = new Mob(player, 5562, new Position(3193, 3961));
			spirit.register();
	...
	private void clickCrate(Player player, GameObject object) {
		if(player.quest.getStage()[index()] == 6 && !player.inventory.contains(7759)) {
			player.dialogueFactory.sendStatement("You search the crate...").sendPlayerChat(Expression.ANGRY, "Wow Billy was in the crate this whole time!").execute().onAction(() -> {
