...
public class RoyaltyProgram {
	...
	public static void append(Player player, int increment) {
		...
		if(levelUp) {
			player.royaltyLevel++;
			player.royalty = 0;
			World.sendMessage("<col=9E4629> RP: " + PlayerRight.getCrown(player) + "<col=9E4629>" + player.getName() + "</col> has just reached prestige <col=9E4629>" + player.royaltyLevel + "</col>.");
		}
		
		player.send(new SendMessage("You have just received <col=9E4629>" + increment + "</col> royalty point" + (suffix) + ". You now have <col=9E4629>" + player.royalty + "</col> point" + (player.royalty > 1 ? "s" : "") + " and are level <col=9E4629>" + player.royaltyLevel + "</col>."));
		...
		player.send(new SendMessage("You have been rewarded with " + amount + " " + name + ".", MessageColor.DARK_GREEN));
	}
	...
	public static void open(Player player) {
		...
		if(Area.inWilderness(player)) {
			player.dialogueFactory.sendStatement("You can not open a Royalty Program while in the wilderness!").execute();
			return;
		}
		...
		if(player.playerAssistant.busy()) {
			...
			for(int index = 0, string = 37116; index < size; index++) {
				...
				int amount = player.royaltyLevel == 0 ? perk.getReward().getAmount() : perk.getReward().getAmount() * player.royaltyLevel;
				
				items[index + 3] = new Item(item.getId(), amount);
				player.send(new SendString("<col=3c50b2>Level: " + perk.getLevel(), string++));
				player.send(new SendString("<col=347043>" + Utility.formatDigits(amount) + "x " + item.getName(), string++));
			}
			
			player.send(new SendString("The Program rewards players for their activity in game. Every 30", 37111));
			player.send(new SendString("minutes everyone is awarded 1 point. Points accumulate until 100 where", 37112));
			player.send(new SendString("you gain a prestige level, points will be reset and you will be eligible", 37113));
			player.send(new SendString("for the rewards again. Reward amounts are multiplied by prestige level.", 37114));
