package com.nardah.content.preset;
...
public class PresetManager {
	...
	public void upload() {
		...
		player.dialogueFactory.sendOption("Upload preset", () -> {
			...
			refresh();
			AchievementHandler.activate(player, AchievementKey.SETUP_PRELOADING_GEAR, 1);
	...
	public void activate() {
		...
		for(int index = 0, id = 0; index <= 6; index++) {
			player.skills.setMaxLevel(id++, preset[slot].getSkill()[index]);
		}
	...
	private void equipment(Preset preset) {
		for(Item item : preset.getEquipment()) {
			...
			if(player.bank.get(tabSlot).getAmount() <= 0)
				continue;
