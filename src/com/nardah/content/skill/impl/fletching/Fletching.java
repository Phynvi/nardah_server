...
import com.nardah.content.skill.impl.fletching.impl.Stringable;
...
public class Fletching extends Skill {
	...
	protected boolean clickButton(Player player, ClickButtonInteractionEvent event) {
		...
		switch(button) {
			...
			case 6212:
				try {
					player.send(new SendInputAmount("Enter amount", 2, input -> start(player, fletchable, 0, Integer.parseInt(input))), true);
				} catch(Exception ex) {
					logger.error(String.format("player=%s error fletching option1: make-x", player.getName()));
					ex.printStackTrace();
				}
				return true;
			...
			case 8890:
				try {
					player.send(new SendInputAmount("Enter amount", 2, input -> start(player, fletchable, 1, Integer.parseInt(input))), true);
				} catch(Exception ex) {
					logger.error(String.format("player=%s error fletching option2: make-x", player.getName()));
					ex.printStackTrace();
				}
				return true;
			...
			case 8894:
				try {
					player.send(new SendInputAmount("Enter amount", 2, input -> start(player, fletchable, 2, Integer.parseInt(input))), true);
				} catch(Exception ex) {
					logger.error(String.format("player=%s error fletching option3: make-x", player.getName()));
					ex.printStackTrace();
				}
				return true;
			...
			case 8918:
				try {
					player.send(new SendInputAmount("Enter amount", 2, input -> start(player, fletchable, 3, Integer.parseInt(input))), true);
				} catch(Exception ex) {
					logger.error(String.format("player=%s error fletching option3: make-x", player.getName()));
					ex.printStackTrace();
				}
