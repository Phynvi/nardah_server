...
import com.nardah.content.skill.impl.ProducingSkillAction;
...
import com.nardah.util.ItemIdentifiers;
...
public final class SmithingArmour extends ProducingSkillAction {
	...
	private SmithingArmour(Player player, SmithingTable definition, int amount) {
		super(player, Optional.empty(), 4, true);
	...
	/**
	 * Opens the smithing itemcontainer and sets all the values for the specified
	 * {code player}.
	 */
	static boolean openInterface(Player player, Item item, GameObject object) {
		if(object.getDefinition().getId() != 2097 && object.getDefinition().getId() != 2672 && object.getDefinition().getId() != 2031) {
			return false;
		}
		...
		if(item.getId() == 11286 || item.getId() == 1540) {
			...
			player.skills.addExperience(Skill.SMITHING, 350);
