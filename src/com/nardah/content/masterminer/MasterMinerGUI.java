...
public class MasterMinerGUI {
	...
	private void updateSpecialButtons() {
		...
		} else if(player.masterMinerTask.getTimeLastCompelted() + MasterMinerTaskHandler.SIXHOUR > System.currentTimeMillis()) {
			player.send(new SendString(Util.toTime(player.masterMinerTask.getTimeLastCompelted() + MasterMinerTaskHandler.SIXHOUR - System.currentTimeMillis()), 24340));
		} else {
	...
	protected void updateGoldCount() {
		...
		player.send(new SendString(Util.toNiceString(data.totalGold), 24336));
	...
	protected void updateMobDetails() {
		for(int i = 0; i < 8; i++) {
			...
			player.send(new SendString("Lvl: " + Util.toNiceString(data.mobData[i].getLevel()), 24407 + i * INCREMENT));
			// player.send(new SendString("Price: " +
			// toNiceString(data.mobData[i].getPrice()), 24408 + i * INCREMENT));
			player.send(new SendTooltip("This does absolutely fucking nothing " + Util.toNiceString(data.mobData[i].getGPS()), 24409 + i * INCREMENT));
	...
	private void populateStatsBar() {
		...
		statsArray[0] = "Gold/sec: " + Util.toNiceString(data.goldPerSec);
