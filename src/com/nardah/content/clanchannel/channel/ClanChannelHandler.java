...
import static com.nardah.content.clanchannel.channel.ClanManagement.*;
...
public class ClanChannelHandler {
	...
	void sendConnectionWarning(Player player, ClanMember member) {
		player.dialogueFactory.sendStatement("-<col=FF0000>WARNING</col>-", "Contributing to this clan or gaining a rank will remove", "all of your progress from your current clan <col=FF4444>" + player.lastClan + "</col>.", "Are you sure you want to join?").sendOption("Yes, I accept.", () -> channel.add(player, member), "No, I don't want to risk it.", () -> player.clanChannel = null).execute();
	}
	...
	public void updateMemberList(ClanMember member) {
		...
		for(int index = 0; index < size * 3; index++) {
			...
			if(nextMember == null || (!nextMember.player.isPresent() && index / 3 >= members.size())) {
				...
				player.send(new SendString("#" + channel.getDetails().getClanRank(nextMember), string++));
				string++;
			}
		}
	}
