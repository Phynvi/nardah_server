package com.nardah.content.clanchannel.channel;
...
import com.nardah.content.clanchannel.content.*;
...
public class ClanChannel implements Comparable<ClanChannel> {
	...
	public void addExperience(double experience) {
		...
		if(details.level != level) {
			...
			message(true, "Woot! Our clan has leveled up to <col=255>" + Utility.formatEnum(level.name()) + "</col>! Total Experience: <col=255>" + Utility.formatDigits(details.experience), "We have earned <col=255>" + level.getPoints() + "</col> CP, which puts us at <col=255>" + Utility.formatDigits(details.points) + "</col> CP.");
		}
	}
	...
	public void activateTask(ClanTaskKey key, String name, int amount) {
		...
		if(progress <= 0) {
			...
			message("Hell yeah! We completed a clan task and received <col=255>" + reward + "</col> points. We now have <col=255>" + Utility.formatDigits(details.points) + "</col> CP.");
		...
		for(ClanAchievement list : ClanAchievement.values()) {
			if(list == achievement) {
				...
				if(details.completedAchievement(list)) {
					message("Hooray! We just completed the achievement:" + list.details, "We have earned " + list.getPoints() + " CP and " + Utility.formatDigits(list.getExperience()) + " Clan EXP.");
				}
			}
		}
	}
	...
	public void splitLoot(Player player, Actor dead, Item item) {
		...
		players.forEach(other -> {
			getMember(other.getName()).ifPresent(member -> member.message(Utility.formatDigits(split) + " coins have been deposited into your bank vault."));
	...
	public JsonObject toJson() {
		...
		for(ClanMember member : members) {
			...
			memberObj.addProperty("mob-kills", member.npcKills);
		...
		new GsonParser("/content/clan/" + owner, false) {
			...
			protected void parse(JsonObject object) {
				...
				for(JsonElement element : members) {
					...
					member.npcKills = memberObj.get("mob-kills").getAsInt();
