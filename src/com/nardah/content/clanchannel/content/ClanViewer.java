...
public class ClanViewer {
	...
	public void open(ClanChannel channel, ClanTab tab) {
		...
		if(tab == ClanTab.OVERVIEW) {
			...
			drawClanDetails();
			interfaceId = 43000;
			player.send(new SendString(name + ":", 43006));
		} else if(tab == ClanTab.MEMBERS) {
			drawClanMembers();
			interfaceId = 43500;
			player.send(new SendString("Member List:", 43006));
		}
		
		player.send(new SendConfig(531, tab.ordinal()));
		player.send(new SendString(viewing != null ? viewing.getName() : "", 43009));
		player.send(new SendString(viewing != null ? viewing.getSlogan() : "", 43010));
		player.send(new SendString(searchKey.isEmpty() ? "Search for clan" : searchKey, 43012));
		player.send(new SendString(name, 43019));
		player.send(new SendItemOnInterface(43011, showcase));
	...
	private void drawClanList(String key) {
		...
		player.send(new SendScrollbar(43300, size * 14));
	}
	...
	private String[] getDetails() {
		...
		return new String[]{"</col>Owner: " + viewing.getColor() + "<clan=7> " + Utility.formatName(viewing.getOwner()), "</col>Established: " + viewing.getColor() + "" + viewing.getDetails().established, "</col>Members: " + viewing.getColor() + "" + viewing.getMembers().size(), "</col>Type: " + viewing.getColor() + viewing.getDetails().type.getIcon() + " " + Utility.formatEnum(viewing.getDetails().type.name()), "</col>Level: " + viewing.getColor() + "" + Utility.formatEnum(viewing.getDetails().level.name()), "</col>Tag " + viewing.getColor() + "" + (viewing.getTag().length() <= 0 ? "None" : ("[" + viewing.getTag()) + "]"), "</col>Experience: " + viewing.getColor() + "" + Utility.formatDigits(viewing.getDetails().experience), "</col>Points: " + viewing.getColor() + "" + Utility.formatDigits(viewing.getDetails().points), "</col>Achievements Completed: " + viewing.getColor() + "NA", "</col>Tasks Completed: " + viewing.getColor() + "NA",
	...
	private String[] getProfile(ClanMember member) {
		if(member == null)
			return new String[10];
		return new String[]{"</col>Rank: " + viewing.getColor() + "" + member.rank.getString() + " " + member.rank.name, "</col>Joined: " + viewing.getColor() + "" + member.joined, "</col>Exp Gained: " + viewing.getColor() + "" + Utility.formatDigits(member.expGained), "</col>PvP Kills: " + viewing.getColor() + "" + member.playerKills, "</col>PvM Kills: " + viewing.getColor() + "" + member.npcKills, "", "", "", "",};
	}
	...
	private void drawClanMembers() {
		...
		player.send(new SendScrollbar(45050, size * 18));
	...
	public void update(ClanChannel channel) {
		if(player.interfaceManager.isInterfaceOpen(43000))
