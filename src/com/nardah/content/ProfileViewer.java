...
import com.nardah.net.packet.out.SendPlayerIndex;
...
public class ProfileViewer {
	...
	public static String[] string(Player player) {
		return new String[]{"Created: ", player.created, "Total play time:", Utility.getTime(player.playTime), "Session time:", Utility.getTime(player.sessionPlayTime), "Networth:", "" + Utility.formatPrice(player.playerAssistant.networth()) + " RT", "Clan:", player.clan == null ? "None" : player.clan, "Total level:", "" + Utility.formatDigits(player.skills.getTotalLevel()), "Kills/Deaths/KDR", "" + player.kill + "/" + player.death + "/" + player.playerAssistant.kdr() + "", "Current KC/Highest KC", "0/0", "Achievements Completed:", "" + AchievementHandler.getTotalCompleted(player) + "", "Quests Completed:", "" + player.quest.getCompleted()};
	}
	...
	public static void open(Player player, Player other) {
		...
		for(int index = 0, string = 51832; index < Skill.SKILL_COUNT; index++, string += 2) {
			...
			player.send(new SendString(skill.getLevel() + "<col=" + color + "></col>/<col=" + color + ">" + skill.getMaxLevel(), string));
		}
