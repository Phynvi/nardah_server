...
public class FightCaves extends Activity {
	...
	/**
	 * A set of mobs in this activity.
	 */
	public final Set<Mob> mobs = new HashSet<>();
	...
	protected void start() {
		...
		for(int id : wave.getMonster()) {
			Mob mob = new Mob(id, RandomUtils.random(boundaries));
			mob.owner = player;
			add(mob);
			mobs.add(mob);
	...
	public void update() {
		...
		if(progress >= 100 && !completed)
			progress = 99;
		ActivityPanel.update(player, progress, "Fight Caves", new Item(6570), "</col>Wave: <col=FF5500>" + (wave.ordinal() + 1) + "/" + (TzhaarData.WaveData.values().length), "</col>Monsters Left: <col=FF5500>" + mobs.size(), "</col>Tokkuls Gained: <col=FF5500>" + Utility.formatDigits(tokkuls), "</col>Time: <col=FF5500>" + Utility.getTime(time));
	}
