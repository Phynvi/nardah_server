...
import com.nardah.game.world.entity.combat.strategy.npc.boss.Zulrah;
...
public enum ZulrahState implements ZulrahInterface<ZulrahActivity> {
	ATTACKING() {
		...
		public void execute(ZulrahActivity activity) {
			...
			if(activity.count == 1) {
				activity.zulrah = new Mob(2042, new Position(2266, 3073));
				activity.attackable = false;
		...
		public void execute(ZulrahActivity activity) {
			...
			if(stage == 0) {
				...
				Mob snake = new Mob(2045, activity.target);
				activity.add(snake);
