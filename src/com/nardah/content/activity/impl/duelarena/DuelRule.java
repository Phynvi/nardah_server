...
import com.nardah.net.packet.out.SendToggle;
...
public enum DuelRule {
	...
	}), NO_FORFEIT(31039, 636, (player, rules) -> true), NO_PRAYER(31040, 637, (player, rules) -> true), NO_DRINKS(31041, 638, (player, rules) -> true), NO_FOOD(31042, 639, (player, rules) -> true), NO_MOVEMENT(31043, 640, new RuleCondition() {
		@Override
		public boolean canSelect(Player player, DuelRules rules) {
	...
	}), LOCK_ITEMS(31059, 643, ((player, rules) -> true)), NO_HEAD(13813, 16384, Equipment.HELM_SLOT, (player, rules) -> true), NO_CAPE(13814, 32768, Equipment.CAPE_SLOT, (player, rules) -> true), NO_NECKLACE(13815, 65536, Equipment.AMULET_SLOT, (player, rules) -> true), NO_AMMO(13816, 134217728, Equipment.ARROWS_SLOT, (player, rules) -> true), NO_WEAPON(13817, 131072, Equipment.WEAPON_SLOT, new RuleCondition() {
		@Override
		public boolean canSelect(Player player, DuelRules rules) {
			if(rules.contains(DuelRule.ONLY_FUN_WEAPONS)) {
	...
	}), NO_BODY(13818, 262144, Equipment.CHEST_SLOT, (player, rules) -> true), NO_SHIELD(13819, 524288, Equipment.SHIELD_SLOT, (player, rules) -> true), NO_LEGS(13820, 2097152, Equipment.LEGS_SLOT, (player, rules) -> true), NO_GLOVES(13823, 8388608, Equipment.HANDS_SLOT, (player, rules) -> true), NO_BOOTS(13822, 16777216, Equipment.FEET_SLOT, (player, rules) -> true), NO_RINGS(13821, 67108864, Equipment.RING_SLOT, (player, rules) -> true);
		...
		if(Activity.search(player, DuelArenaActivity.class).isPresent()) {
			...
			for(DuelRule rule : DuelRule.values()) {
				if(!EQUIPMENT_RULES.contains(rule)) {
					...
					activity.getOther(player).ifPresent(it -> it.send(new SendConfig(rule.getValue(), activity.getRules().contains(rule) ? 1 : 0)));
				}
			}
