package com.nardah.content.activity.record;
...
public class PlayerRecord {
	...
	public long end(ActivityType activity, boolean track) {
		...
		if(track && !PlayerRight.isPriviledged(player)) {
			GameRecord tracker = new GameRecord(player, record, activity);
			System.out.println("Adding player record");
	...
	void clean(int size) {
		int string = 32401;
		for(int count = 0; count < (size < 10 ? 10 : size); count++) {
