...
public final class DialogueFactory {
	 ...
	 * Logger.
	 */
	private static final Logger dialogueLogger = LogManager.getLogger(LoggerType.CONTENT);
	...
	final DialogueFactory sendPlayerChat(PlayerDialogue dialogue) {
		...
		switch(lines.length) {
			...
			case 4:
				...
				break;
			default:
				dialogueLogger.error(String.format("Invalid player dialogue line length: %s", lines.length));
				break;
		}
		return this;
	}
	...
	/**
	 * Sends a dialogue with a mob talking.
	...
	final DialogueFactory sendNpcChat(NpcDialogue dialogue) {
		...
		final MobDefinition npcDef = MobDefinition.get(id);
		...
		switch(lines.length) {
			...
			case 4:
				...
				break;
			default:
				dialogueLogger.error("Invalid " + npcDef.getName() + " mob dialogue line length: " + lines.length);
				break;
		}
		return this;
	}
	...
	final DialogueFactory sendStatement(StatementDialogue dialogue) {
		...
		switch(dialogue.getLines().length) {
			...
			case 5:
				...
				break;
			default:
				dialogueLogger.error("Invalid statement dialogue line length: " + dialogue.getLines().length);
				break;
		}
		return this;
	}
	...
	final DialogueFactory sendInformationBox(InformationDialogue dialogue) {
		...
		switch(dialogue.getLines().length) {
			...
			case 4:
				...
				break;
			default:
				dialogueLogger.error("Invalid information dialogue line length: " + dialogue.getLines().length);
				break;
		}
		return this;
	}
	 ...
	 * @throws IllegalStateException if any lines of the text exceed a certain
	 * length.
	 */
