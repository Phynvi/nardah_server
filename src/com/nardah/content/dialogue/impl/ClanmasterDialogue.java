...
import com.nardah.content.clanchannel.content.ClanTask;
...
public class ClanmasterDialogue extends Dialogue {
	...
	public void sendDialogues(DialogueFactory factory) {
		...
		factory.sendOption("World Clan List", () -> player.clanViewer.open(ClanViewer.ClanTab.OVERVIEW), "Clan Management", () -> clanManagement(factory), "Clan Levels", () -> clanLevel(factory.getPlayer()), "Nevermind", factory::clear);
		factory.execute();
	}
	...
	private void clanManagement(DialogueFactory factory) {
		...
		if(!member.isPresent() || !channel.canManage(member.get())) {
			factory.sendNpcChat(1143, "You do not have the proper authorization to manage", "the clan " + channel.getName() + "! If you want to manage your own clan", "please join your own clan and then speak to me again.");
			return;
		}
	...
	private void clanTask(ClanChannel channel, DialogueFactory factory) {
		...
		factory.sendOption("Task Information", () -> {
			...
			if(task != null) {
				factory.sendNpcChat(1143, "Your clan currently has a task assigned!", "Cancel or complete it to obtain another one.");
				return;
			}
			...
			if(channel.getDetails().points < 5) {
				factory.sendStatement("You do not have 5 clan points.", "Cancelling your task will set you back to zero.");
			}
			...
			channel.message("Our clan task was cancelled for 5 CP. We have <col=255>" + Utility.formatDigits(channel.getDetails().points) + "</col> remaining.");
