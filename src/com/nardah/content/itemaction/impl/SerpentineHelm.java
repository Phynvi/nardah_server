...
public class SerpentineHelm extends ItemAction {
	...
	public boolean itemOnItem(Player player, Item first, Item second) {
		...
		if((first.matchesId(CHARGED_HELM) && second.matchesId(TANZ_MUTAGEN)) || (second.matchesId(CHARGED_HELM) && first.matchesId(TANZ_MUTAGEN))) {
			player.message("You need to uncharge your Serpentine helm first.");
			return true;
		}

		if((first.matchesId(CHARGED_HELM) && second.matchesId(MAGMA_MUTAGEN)) || (second.matchesId(CHARGED_HELM) && first.matchesId(MAGMA_MUTAGEN))) {
			player.message("You need to uncharge your Serpentine helm first.");
			return true;
		}

		if((first.matchesId(UNCHARGED_HELM) && second.matchesId(TANZ_MUTAGEN)) || (second.matchesId(UNCHARGED_HELM) && first.matchesId(TANZ_MUTAGEN))) {
			player.inventory.remove(TANZ_MUTAGEN);
			player.inventory.replace(UNCHARGED_HELM, 13_196, true);
			player.message("You recolor your Serpentine helm into a Tanzanite helm!");
			return true;
		}

		if((first.matchesId(UNCHARGED_HELM) && second.matchesId(MAGMA_MUTAGEN)) || (second.matchesId(UNCHARGED_HELM) && first.matchesId(MAGMA_MUTAGEN))) {
			player.inventory.remove(MAGMA_MUTAGEN);
			player.inventory.replace(UNCHARGED_HELM, 13_198, true);
			player.message("You recolor your Serpentine helm into a Magma helm!");
			return true;
		}
