...
import com.nardah.net.packet.out.SendChatBoxInterface;
...
public class TanzaniteHelm extends ItemAction {
	private static final short FULL = 11_000;

	private static final short ZULRAH_SCALES = 12_934;

	private static final short UNCHARGED_HELM = 13_196;
	private static final short CHARGED_HELM = 13_197;

	private static final DecimalFormat FORMATTER = new DecimalFormat("#.#");

	public TanzaniteHelm() {
		FORMATTER.setRoundingMode(RoundingMode.FLOOR);
	}

	@Override
	public String name() {
		return "Tanzanite Helm";
	}
	...
	public static void check(Player player) {
		String scales = FORMATTER.format(player.tanzaniteHelmCharges * 100 / FULL) + "%";
		player.message("Charges: <col=007F00>" + scales);
	}

	private static void charge(Player player, Item helm, Item scales) {
		if(player.tanzaniteHelmCharges >= FULL) {
			if(helm.matchesId(UNCHARGED_HELM)) {
				player.inventory.replace(UNCHARGED_HELM, CHARGED_HELM, true);
			}
			player.message("Your Tanzanite helm is already fully charged.");
			return;
		}

		if(Integer.MAX_VALUE - scales.getAmount() < FULL) {
			scales = scales.copy();
			scales.setAmount(FULL - player.tanzaniteHelmCharges);
		}

		if(player.tanzaniteHelmCharges + scales.getAmount() > FULL) {
			scales = scales.copy();
			scales.setAmount(FULL - player.tanzaniteHelmCharges);
		}

		player.tanzaniteHelmCharges += scales.getAmount();
		player.inventory.remove(scales);
		player.inventory.replace(UNCHARGED_HELM, CHARGED_HELM, true);
		player.message(String.format("You load %s scales into your Tanzanite helm.", Utility.formatDigits(scales.getAmount())));
	}

	private static void uncharge(Player player) {
		Item scales = new Item(ZULRAH_SCALES, player.tanzaniteHelmCharges);
		...
		player.message(String.format("You uncharged your Tanzanite helm and received %s of Zulra's scales.", Utility.formatDigits(scales.getAmount())));
	}
