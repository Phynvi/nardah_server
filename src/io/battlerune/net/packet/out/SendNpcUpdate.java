...
public final class SendNpcUpdate extends OutgoingPacket {
	...
	private static void updateMovement(PacketBuilder packet, Npc npc) {
		...
		if(npc.movement.getRunningDirection() != -1) {
			packet.writeBit(true).writeBits(2, 2).writeBits(3, npc.movement.getWalkingDirection()).writeBits(3, npc.movement.getRunningDirection()).writeBit(npc.isUpdateRequired());
		} else if(npc.movement.getWalkingDirection() != -1) {
			packet.writeBit(true).writeBits(2, 1).writeBits(3, npc.movement.getWalkingDirection()).writeBit(npc.isUpdateRequired());
		} else {
