package io.battlerune.game.event.bus;

import io.battlerune.game.event.Event;
import io.battlerune.game.event.listener.EventListener;
 ...
 * {@link EventListener}'s must subscribe to the bus in order to receive
 * {@link Event}'s send by the bus. {@link EventListener}'s can then decide
 * which {@link Event}'s they are interested in.
 * </p>
 * @author nshusa
...
public final class DataBus {
	 ...
	 * If a listener is subscribed to this bus, then the listener can choose which
	 * {@link Event}'s its interested in.
	 * @param listener The listener to subscribe to this bus.
	 */
	 ...
	 * If a listener is no longer subscribed to this bus then the listener will no
	 * longer receive any events sent from the bus.
	 * @param listener The listener to unsubscribe from this bus.
	 */
	 ...
	 * @param event The event that will be sent to all listeners.
	 */
