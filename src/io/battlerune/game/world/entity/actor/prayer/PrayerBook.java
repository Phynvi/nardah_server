...
public class PrayerBook {
	...
	/**
	 * The amount of ticks a prayer has been enabled for.
	 */
	public int ticks;
	...
	private void activate(Prayer... prayers) {
		for(Prayer prayer : prayers) {
			if(!active.add(prayer) || sendActivated == null)
				continue;
	...
	public void deactivate(Prayer... prayers) {
		for(Prayer prayer : prayers) {
			if(!active.remove(prayer) || sendDeactivated == null)
				continue;
	 ...
	 * activated. Activating some prayers will cause others to deactivate based on
	 * their {@linkplain Prayer.Type}.
	...
	/**
	 * Calculates the amount of prayer points to drain relative to the given game
	 * ticks.
	 ...
	 * @param sendActivated The consumer that sends the activated prayer update to
	 * the client.
	 * @param sendDeactivated The consumer that sends the deactivated prayer update
	 * to the client.
	 * @param sendOverhead The consumer that sends the prayer head icon update to
	 * the client.
	 * @param sendNoOverhead The consumer that removes the prayer head icon from
	 * the client.
	 */
