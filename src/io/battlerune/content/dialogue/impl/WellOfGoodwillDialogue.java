...
public class WellOfGoodwillDialogue extends Dialogue {
	...
	public void sendDialogues(DialogueFactory factory) {
		factory.sendStatement("Greetings " + factory.getPlayer().getName() + ".", "How may the well be of assistance today?").sendOption("How does this work?", () -> {
			factory.onAction(() -> {
				factory.sendStatement("Players may all contribute as much gold as they desire", "towards the well. In return, once the well reaches", Utility.formatDigits(WellOfGoodwill.MAXIMUM) + " the entire server will be granted").sendStatement("double experience for 30minutes.", "After those 30 minutes the well will need to be replenished.", "Well data is saved and stores during server restarts and shutdowns.", "Ensuring safety for contributors.").execute();
			});
		}, "Check time left", () -> {
