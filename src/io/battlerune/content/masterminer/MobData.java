...
public class MobData {
	...
	public void buy(Player player) {
		...
		if(amount == -1) // Max
		{
			...
			if(player.masterMinerData.totalGold >= finalPrice) {
				...
				player.dialogueFactory.sendStatement("Sorry, this action would cost " + Util.toNiceString(finalPrice) + " gold at your level.").execute();
	...
	private void openConfirmationChat(Player player, long finalPrice, int amount) {
		player.dialogueFactory.sendStatement("Are you sure you wish to purchase " + amount + " " + name + "s?", "This action will cost you " + Util.toNiceString(finalPrice) + " gold.", name + "s" + " give " + GPS + " gold per second each.").sendOption("Yes!", () -> {
	...
	private void openMaxConfirmationChat(Player player) {
		player.dialogueFactory.sendStatement("Are you sure you wish to purchase as many " + name + "s as you can?", name + "s" + " give " + GPS + " gold per second each.").sendOption("Yes!", () -> {
	...
	public static MobData fromString(String toParse) {
		...
		return new MobData(parsed[0], Long.parseLong(parsed[1]), Long.parseLong(parsed[2]), Integer.parseInt(parsed[3]), Long.parseLong(parsed[4]));
	}
