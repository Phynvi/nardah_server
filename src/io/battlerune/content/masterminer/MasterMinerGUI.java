...
public class MasterMinerGUI {
	...
	private void updateSpecialButtons() {
		...
		if(player.masterMinerTask.getCurrentTask().isComplete()) {
			...
			player.send(new SendString(toTime(player.masterMinerTask.getTimeLastCompelted() + SIXHOUR - System.currentTimeMillis()), 24340));
		} else {
	...
	protected void updateMobDetails() {
		for(int i = 0; i < 8; i++) {
			...
			player.send(new SendTooltip("This does absolutely fucking nothing " + toNiceString(data.mobData[i].getGPS()), 24409 + i * INCREMENT));
