...
public class MasterMinerTaskHandler {
	...
	public void obtainTask(Player player) {
		if(currentTask.getID() > 0) {
			...
			DialogueFactory factory = new DialogueFactory(player);
			factory.sendNpcChat(250, Expression.DEFAULT, "You must wait another " + toTimeLong(player.masterMinerTask.getTimeLastCompelted() + SIXHOUR - System.currentTimeMillis()), "before getting another task!");
			factory.execute();
			return;
		}
		currentTask = new Task();
	...
	public void claimReward(Player player) {
		...
		player.send(new SendMessage("Successfully completed a task! You've been awarded " + toNiceString(reward) + " extra gold!"));
