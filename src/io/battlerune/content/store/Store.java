...
public abstract class Store {
	...
	protected final void sell(Player player, Item item, int slot, boolean addX) {
		...
		if(sellType() == SellType.NONE) {
			player.send(new SendMessage("[" + name + "] wont buy any items."));
			return;
		}
	...
	protected final void sendSellValue(Player player, int slot) {
		...
		if(value <= 0) {
			player.send(new SendMessage(String.format("[%s] will buy %s for free!", name, item.getName())));
			return;
		}
		
		final String message = this.sellType() != SellType.NONE ? String.format("[%s] will buy %s for %s %s.", name, item.getName(), Utility.formatDigits(value), currencyType.toString()) : String.format("[%s] will not buy any items.", name);
	...
	protected void sendPurchaseValue(Player player, int slot) {
		...
		if(item instanceof StoreItem) {
			...
			final int value = storeItem.getShopValue();
			String message = "[" + name + "] will sell " + item.getName() + " for " + (value <= 0 ? "free!" : Utility.formatDigits(value) + " " + storeItem.getShopCurrency(this).toString() + ".");
			player.send(new SendMessage(message));
		}
