...
public class MysteryBoxEvent extends TickableTask {
	...
	private void reward() {
		...
		if(type != 1) {
			World.sendMessage("[<col=5739B3>Runity Reality@bla@]<col=" + player.right.getColor() + ">" + player.getName() + " </col>has won " + Utility.getAOrAn(reward.getName()) + " <col=5739B3>" + reward.getName() + " </col>from the <col=5739B3>" + boxName + "</col>.");
		}
