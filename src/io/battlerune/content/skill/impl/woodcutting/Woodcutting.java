	...
	protected boolean clickObject(Player player, ObjectInteractionEvent event) {
		...
		// TREE
		{
			...
			if(left == null) {
				...
				if(left > 1000) {
					player.send(new SendMessage("The AFK tree has " + left / 1000 + "," + (left.toString().substring(left.toString().length() - 3, left.toString().length())) + " logs left.."));
				} else {
		...
		if(!player.skills.getSkills()[Skill.WOODCUTTING].reqLevel(tree.levelRequired)) {
			if(event.getType() == ObjectInteractionEvent.InteractionType.FIRST_CLICK_OBJECT)
				player.send(new SendMessage("You need a level of " + tree.levelRequired + " to cut " + StringUtils.appendIndefiniteArticle(event.getObject().getDefinition().name) + "."));
			return true;
		}
