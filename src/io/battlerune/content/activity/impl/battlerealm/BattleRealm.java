...
public class BattleRealm extends GroupActivity {
	...
	protected boolean clickObject(Player player, ObjectInteractionEvent event) {
		...
		switch(event.getObject().getId()) {
			...
			default:
				System.out.println(player.getName() + " clicked on " + event.getObject().getDefinition().getName() + "(id=" + event.getObject().getId() + ")");
		}
		return false;
	}
	...
	public void sequence() {
		super.sequence();
		forEachActivity((mob, activity) -> activity.getPanel().ifPresent(panel -> ((BattleRealmPanel) panel).update((BattleRealmNode) activity)));
	}
}
