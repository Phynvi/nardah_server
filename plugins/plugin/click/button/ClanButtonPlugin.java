...
public class ClanButtonPlugin extends PluginContext {
	...
	protected boolean onClick(Player player, int button) {
		...
		if (button >= -22334 && button <= -22310) {
			List<ClanChannel> clans = player.clanViewer.clanList;
			if (clans.isEmpty())
				return true;
			int index = (button - -22334) / 2;
			if (index < clans.size()) {
		...
		case -32011:
			...
			player.forClan(channel -> channel.getMember(player.getName())
					.ifPresent(member -> channel.getHandler().updateMemberList(member)));
			return true;
		case -32010:
