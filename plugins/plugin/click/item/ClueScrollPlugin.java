		...
		for (int index = 0, count = 0; index < length; index++, count++) {
			...
			for (String announcement : ITEMS_ANNOUNCED) {
				if (name.contains(announcement)) {
					World.sendMessage("<col=" + player.right.getColor() + ">" + player.getName()
							+ " </col>has received " + Utility.getAOrAn(name) + " <col=5739B3>" + name
							+ " </col>from <col=5739B3>" + item.getName().toLowerCase() + "</col>.");
				}
			}
		}
